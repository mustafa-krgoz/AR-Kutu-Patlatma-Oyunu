...
//bascially instantiated an anchor on the host and remembered its location and instantiate a completely separate object on the client 
//just in the same world coordinate
...
namespace GoogleARCore.Examples.CloudAnchor
{
    ...
    /// </summary>
    public class CloudAnchorController : NetworkBehaviour
    {
        /// <summary>
        ...
        /// </summary>
		/// decide what kind of device it is and enable either ARCore or ARKit
        public void Start()
        {
		...
		void CmdSpawnMyUnit()
		{
			...
			var m_connection = andyObject.GetComponent<NetworkIdentity> ().connectionToClient;

			// Now that the object exists on the server, propagate it to all
			// the clients (and also wire up the NetworkIdentity)
			NetworkServer.SpawnWithClientAuthority(andyObject, m_connection);
		}
